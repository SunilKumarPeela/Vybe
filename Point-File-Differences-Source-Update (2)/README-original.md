# Point v8.20.30 — Deterministic Group Membership

After an identity resolves to Display Name, Point now queries indexed original
Members columns directly and expands every exact matching Group Name. This path
does not depend on automatic relationship overlap, uniqueness inference, or a
Relationship Manager rule. It uses the same exact semicolon-token lookup that
direct Members searches use, retains result safety limits, and deduplicates
