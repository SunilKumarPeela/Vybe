...
cl /nologo /std:c++20 /utf-8 /EHsc /W4 /WX /sdl /guard:cf ^
  ...
  src\point_browser_fetcher.cpp src\point_compliance.cpp build\point.res /Fe:build\PointBrowserFetcher.exe ^
...
cl /nologo /std:c++20 /utf-8 /EHsc /W4 /WX /sdl /guard:cf ^
  ...
  src\point_fetcher.cpp src\point_compliance.cpp build\point.res /Fe:build\PointFetcher.exe ^
  /link /DYNAMICBASE /NXCOMPAT /GUARD:CF ^
...
if exist sample\*.csv if not exist build\Inbox\Users.csv copy /Y sample\*.csv build\Inbox\ >nul
