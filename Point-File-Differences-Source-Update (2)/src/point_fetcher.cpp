...
bool supported_name(const std::wstring& name) {
    return point::security_rules::safe_report_name(name);
}

bool valid_api_header(const std::wstring& value) {
    if (value.empty()) return false;
    return std::all_of(value.begin(), value.end(), [](wchar_t ch) {
        return (ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L'z') || (ch >= L'0' && ch <= L'9') || ch == L'-';
    });
}
...
void save_config() {
    ...
    point::compliance::write_user_protected_file(root / "point-fetcher.protected", output.str());
}

void load_config() {
    const auto path = root / "point-fetcher.protected";
    if (!std::filesystem::exists(path)) return;
    const auto protected_text = point::compliance::read_user_protected_file(path);
    std::istringstream input(protected_text, std::ios::in | std::ios::binary);
    char signature[17]{};
    input.read(signature, sizeof(signature));
    if (!input || !std::equal(std::begin(signature), std::end(signature), "POINT_FETCHER_V1")) return;
    ...
    for (std::uint32_t index = 0; index < count; ++index) {
        ...
        if (auth < 0 || auth > 3 || schedule.interval_hours < 1 ||
            ...
            (auth != 0 && schedule.credential_target != credential_id(schedule.url, schedule.filename)))
            continue;
...
std::wstring download(const DownloadSchedule& schedule) {
    if (!supported_name(schedule.filename)) throw std::runtime_error("Unsafe download filename rejected");
    if (schedule.auth != AuthMode::Public && schedule.credential_target != credential_id(schedule.url, schedule.filename))
        throw std::runtime_error("Credential does not belong to this URL and filename");
    ...
    if (!WinHttpSetOption(request, WINHTTP_OPTION_REDIRECT_POLICY, &redirect_policy, sizeof(redirect_policy)))
        throw std::runtime_error("Could not enforce the HTTPS redirect policy");
    std::wstring headers;
    ...
    for (;;) {
        ...
        if (!received) break;
        total += received;
        if (total > 500ULL * 1024ULL * 1024ULL)
            throw std::runtime_error("Download exceeds the 500 MiB import limit");
        output.write(buffer.data(), received);
        if (!output) throw std::runtime_error("Download write failed");
    }
    output.close();
    if (!output) throw std::runtime_error("Download could not be finalized");
...
void add_schedule() {
    ...
    if (schedule.filename.empty() ||
        ...
        throw std::runtime_error("Enter a safe .csv, .xlsx filename");
    ...
    case WM_FETCH_COMPLETE: {
        ...
        if (result && result->index < schedules.size()) {
            ...
            if (result->browser_fallback) {
                const auto browser = point::compliance::application_directory() / L"PointBrowserFetcher.exe";
                ...
                const auto launched = reinterpret_cast<INT_PTR>(ShellExecuteW(
                    ...
                    point::compliance::application_directory().c_str(), SW_SHOWNORMAL));
                if (launched > 32) {
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    try {
        const auto app_root = point::compliance::data_directory();
        ...
        MessageBoxW(nullptr, L"Fetcher startup denied. Check Windows-group membership, point-security.conf, and protected Fetcher configuration.",
            L"Point Fetcher", MB_ICONERROR);
        return 1;
    }
