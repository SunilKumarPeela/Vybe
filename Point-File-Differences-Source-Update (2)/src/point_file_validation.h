...
inline bool looks_like_html(std::string text) {
    ...
    text.erase(0, first);
    return text.rfind("<!doctype html", 0) == 0 || text.rfind("<html", 0) == 0 ||
           text.rfind("<head", 0) == 0 || text.rfind("<body", 0) == 0;
}
...
inline bool valid_office_zip(const std::filesystem::path& path) {
    ...
    if (eocd + 22U + u16(tail.data() + eocd + 20) != tail.size() ||
        u16(tail.data() + eocd + 4) != 0 || u16(tail.data() + eocd + 6) != 0 ||
        u16(tail.data() + eocd + 8) != u16(tail.data() + eocd + 10)) return false;
    ...
    for (std::uint32_t index = 0; index < entries; ++index) {
        ...
        if (cursor < 0 || static_cast<std::uint64_t>(static_cast<std::streamoff>(cursor)) >
            static_cast<std::uint64_t>(central_offset) + central_size) return false;
        const auto unpacked = u32(header + 24);
        if (unpacked == 0xFFFFFFFFU || unpacked > 512U * 1024U * 1024U) return false;
        unpacked_total += unpacked;
        if (unpacked_total > 2ULL * 1024ULL * 1024ULL * 1024ULL) return false;
        ...
        std::transform(normalized.begin(), normalized.end(), normalized.begin(),
            [](unsigned char ch) { return static_cast<char>(std::tolower(ch)); });
        if (normalized.find("vbaproject") != std::string::npos ||
            normalized.rfind("xl/macrosheets/", 0) == 0 ||
        ...
        if (!input || input.tellg() < 0 ||
            static_cast<std::uint64_t>(static_cast<std::streamoff>(input.tellg())) >
            static_cast<std::uint64_t>(central_offset) + central_size) return false;
    }
...
inline bool validate(const std::filesystem::path& path,
    ...
    if (error || size == 0 || size > 500ULL * 1024ULL * 1024ULL) {
        reason = L"The downloaded file is empty or exceeds the 500 MiB import limit.";
        return false;
    }
    const std::wstring extension = lower_extension(expected_name);
    if (extension != L".csv" && extension != L".xlsx") {
        reason = L"Only CSV and XLSX downloads are accepted.";
        return false;
    }
    std::ifstream input(path, std::ios::binary);
    if (!input) {
