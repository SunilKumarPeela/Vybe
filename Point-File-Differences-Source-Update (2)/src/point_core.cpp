...
std::string last_audit_hash(const std::filesystem::path& path) {
    ...
    if (!input) {
        if (std::filesystem::exists(path)) throw std::runtime_error("Unable to read audit log.");
        return std::string(64, '0');
    }
    input.seekg(0, std::ios::end);
    const auto size = input.tellg();
    if (size < 0) throw std::runtime_error("Unable to read audit-log size.");
    if (size == 0) return std::string(64, '0');
    const auto length = std::min<std::streamoff>(static_cast<std::streamoff>(size), 8192);
    input.seekg(-length, std::ios::end);
    std::string tail(static_cast<std::size_t>(length), '\0');
    input.read(tail.data(), length);
    if (!input || tail.back() != '\n') throw std::runtime_error("Audit log ends with an incomplete record.");
    tail.pop_back();
    const auto last = tail.find_last_of('\t');
    if (last == std::string::npos) throw std::runtime_error("Audit log tail is invalid.");
    const auto hash = tail.substr(last + 1);
    if (hash.size() != 64 || !std::all_of(hash.begin(), hash.end(), [](unsigned char ch) {
            return (ch >= '0' && ch <= '9') || (ch >= 'a' && ch <= 'f');
        })) throw std::runtime_error("Audit log tail hash is invalid.");
    return hash;
}
...
bool looks_like_payment_card_number(const std::string& value) {
    auto valid = [](const std::string& digits) {
        if (digits.size() < 13 || digits.size() > 19) return false;
        int sum = 0; bool double_digit = false;
        for (auto it = digits.rbegin(); it != digits.rend(); ++it) {
            int digit = *it - '0';
            if (double_digit) { digit *= 2; if (digit > 9) digit -= 9; }
            sum += digit; double_digit = !double_digit;
        }
        return sum % 10 == 0;
    };
    std::string digits;
...
QueryResult Engine::query(const QueryRequest& request) const {
    ...
    if (display_output != request.output_fields.end() &&
        group_output != request.output_fields.end()) {
        const auto display_column = static_cast<std::size_t>(std::distance(
        ...
        for (const auto& existing : result.rows) {
            if (display_column >= existing.size() ||
                group_column >= existing.size() ||
                trim(existing[display_column]).empty() ||
                !trim(existing[group_column]).empty()) {
                expanded_rows.push_back(existing);
                continue;
            }
            std::set<std::pair<std::size_t, std::size_t>> member_matches;
            const auto member_keys = indexed_field_values(
                "displayname", existing[display_column]);
            for (std::size_t dataset_id = 0;
                 dataset_id < datasets_.size(); ++dataset_id) {
                const auto& dataset = datasets_[dataset_id];
...
QueryResult Engine::compare_profiles(
    ...
    auto append_row = [&](const std::string& field,
        ...
        result.rows.push_back({
            field, category, is_highly_sensitive_field(field) ? mask_sensitive_value(item.display) : item.display, joined_sources(sources),
            risk, action});
        if (result.rows.size() > limits_.max_result_rows)
            throw std::runtime_error(
        ...
        for (std::size_t row_id = 0;
            ...
            for (std::size_t column = 0;
                ...
                if (details.size() < 4096) {
                    ...
                    details += dataset.headers[column] + "=" +
                        (is_highly_sensitive_field(dataset.headers[column]) ? mask_sensitive_value(value) : value);
                }
                if (details.size() > 4096) {
                    details.resize(4093);
                    details += "...";
...
QueryResult Engine::changes_since(
    ...
    auto row_detail = [](const DataSet& dataset,
        ...
        for (std::size_t column = 0;
            ...
            if (column < row.size()) detail += is_highly_sensitive_field(dataset.headers[column])
                ? mask_sensitive_value(trim(row[column])) : trim(row[column]);
            if (detail.size() > 4096) {
                detail.resize(4093);
                detail += "...";
                break;
...
void Engine::export_csv(const QueryResult& result,
    ...
    if (std::filesystem::exists(destination) && is_link_like(destination))
        throw std::runtime_error("Linked export destination rejected.");
    auto temporary = destination;
#ifdef _WIN32
    temporary += ".point-export-" + std::to_string(GetCurrentProcessId()) + "-" + std::to_string(GetCurrentThreadId()) + ".tmp";
#else
    temporary += ".point-export.tmp";
#endif
    if (std::filesystem::exists(temporary) && is_link_like(temporary))
        throw std::runtime_error("Linked export temporary file rejected.");
    struct ExportCleanup {
        std::filesystem::path path;
        ~ExportCleanup() { std::error_code ignored; std::filesystem::remove(path, ignored); }
    } cleanup{temporary};
    ...
    if (!MoveFileExW(temporary.c_str(), destination.c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH))
        throw std::runtime_error("Unable to publish export; close the previous file and retry.");
#else
...
void append_audit(const std::filesystem::path& root,
    ...
    std::lock_guard<std::mutex> thread_lock(write_mutex);
#ifdef _WIN32
    struct AuditLock {
        HANDLE handle = INVALID_HANDLE_VALUE;
        ~AuditLock() { if (handle != INVALID_HANDLE_VALUE) CloseHandle(handle); }
    } process_lock;
    const auto lock_path = root / "Logs" / "point-audit.lock";
    for (int attempt = 0; attempt < 100; ++attempt) {
        process_lock.handle = CreateFileW(lock_path.c_str(), GENERIC_READ | GENERIC_WRITE,
            FILE_SHARE_READ, nullptr, OPEN_ALWAYS, FILE_FLAG_OPEN_REPARSE_POINT, nullptr);
        if (process_lock.handle != INVALID_HANDLE_VALUE) break;
        if (GetLastError() != ERROR_SHARING_VIOLATION) break;
        Sleep(10);
    }
    if (process_lock.handle == INVALID_HANDLE_VALUE) throw std::runtime_error("Audit log cannot be locked for writing.");
    BY_HANDLE_FILE_INFORMATION lock_info{};
    if (!GetFileInformationByHandle(process_lock.handle, &lock_info) ||
        (lock_info.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) != 0)
        throw std::runtime_error("Audit lock file is invalid.");
#endif
    const auto timestamp = timestamp_utc();
    const auto path = root / "Logs" / ("point-audit-" + timestamp.substr(0, 10) + ".log");
    if (std::filesystem::exists(path) && is_link_like(path))
        throw std::runtime_error("Linked audit log rejected.");
    const auto safe_event = audit_safe(event);
    const auto safe_detail = audit_safe(detail);
    const auto previous = last_audit_hash(path);
    const auto digest = audit_digest(previous + "|" + timestamp + "|" + safe_event + "|" + safe_detail);
    std::ofstream log(path, std::ios::binary | std::ios::app);
    if (!log) throw std::runtime_error("Audit log is not writable.");
    log << timestamp << '\t' << safe_event << '\t' << safe_detail
        << '\t' << previous << '\t' << digest << '\n';
    log.close();
    if (!log) throw std::runtime_error("Audit log write failed.");
}
