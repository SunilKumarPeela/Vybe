...
constexpr int ID_MENU_FILE_DIFFERENCES = 234;
constexpr int ID_MENU_COMPARE_FILES = 235;
...
constexpr UINT_PTR INBOX_WATCH_TIMER_ID = 4;
HANDLE inbox_change_handle = INVALID_HANDLE_VALUE;
std::time_t last_retention_run = 0;
...
void save_field_synonyms(
    ...
    point::compliance::write_user_protected_file(temporary, serialize_field_synonyms(groups));
    if (!MoveFileExW(
...
void save_user_relationships(
    ...
    point::compliance::write_user_protected_file(temporary, content);
...
bool copy_selected_range();
bool copy_active_cell(HWND focus);
...
LRESULT CALLBACK grid_edit_subclass_impl(
        HWND control, UINT message, WPARAM wparam, LPARAM lparam,
        UINT_PTR, DWORD_PTR) {
    if (message == WM_COPY || message == WM_CUT) {
        ...
        if (copy_active_cell(control) && message == WM_CUT && start != end)
            SendMessageW(control, EM_REPLACESEL, TRUE, reinterpret_cast<LPARAM>(L""));
        return 0;
    }
    switch (message) {
    ...
    case WM_CONTEXTMENU: {
        ...
        case MENU_CUT:
            if (selection_active) {
                if (copy_selected_range()) clear_selected_range();
            } else {
...
LRESULT CALLBACK grid_edit_subclass(HWND control, UINT message, WPARAM wparam, LPARAM lparam, UINT_PTR id, DWORD_PTR data) {
    try { return grid_edit_subclass_impl(control, message, wparam, lparam, id, data); }
    catch (const std::exception& ex) {
        MessageBoxW(main_window, widen(ex.what()).c_str(), L"Point operation failed", MB_ICONERROR);
        return 0;
    }
}
...
void toggle_query_mode() {
    const bool open_file_differences = change_mode;
    hide_suggestions();
    commit_visible_cells();
    ...
    if (open_file_differences) {
        point::file_diff_ui::open(main_window, [] { return engine.get(); }, app_root);
    }
}
...
void save_auto_import_schedules() {
    ...
    point::compliance::write_user_protected_file(temporary, output.str());
    if (!MoveFileExW(temporary.c_str(), path.c_str(),
            MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH)) {
        std::error_code ignored;
        std::filesystem::remove(temporary, ignored);
...
void add_auto_import_schedules() {
    ...
    const COMDLG_FILTERSPEC filters[] = {
        {L"CSV and Excel files", L"*.csv;*.xlsx"},
        {L"All files", L"*.*"}};
...
void choose_input_files() {
    ...
    dialog.lpstrFilter = L"Supported data files (*.csv;*.xlsx)\0"
        L"*.csv;*.xlsx\0All files (*.*)\0*.*\0\0";
...
void open_count_drilldown(int logical_row) {
    ...
    try {
        ...
        const auto policy = point::compliance::load_policy(point::compliance::application_directory());
        point::compliance::authorize_current_user(policy);
        export_authorized = point::compliance::current_user_can_export(policy);
        if (export_authorized) engine->export_csv(detail, export_path);
...
void export_grid() {
    ...
    try {
        const auto policy = point::compliance::load_policy(point::compliance::application_directory());
        point::compliance::authorize_current_user(policy);
        if (!point::compliance::current_user_can_export(policy))
            throw std::runtime_error(
...
bool write_clipboard_text(const std::wstring& text) {
    ...
    point::compliance::authorize_current_user(policy);
    if (!point::compliance::current_user_can_export(policy))
        throw std::runtime_error("Clipboard export requires Point Exporters membership.");
    if (text.size() > 50ull * 1024ull * 1024ull / sizeof(wchar_t))
        throw std::runtime_error("Clipboard exceeds the 50 MiB safety limit.");
    point::append_audit(app_root, "CLIPBOARD_EXPORT", "authorized export requested");
...
std::wstring clipboard_safe_cell(std::wstring value, const std::string& header = {}) {
    auto raw = narrow(value);
    if (point::looks_like_payment_card_number(point::trim(raw)))
        throw std::runtime_error("Clipboard blocked: possible payment card number detected.");
    if (point::is_highly_sensitive_field(header)) raw = point::mask_sensitive_value(raw);
    raw = point::escape_csv_for_spreadsheet(raw);
    value = widen(raw);
    for (wchar_t& ch : value) if (ch == L'\t' || ch == L'\r' || ch == L'\n') ch = L' ';
    return value;
}
    ...
    try {
        for (int row = first_row; row <= last_row; ++row) {
            for (int column = first_column;
                ...
                clipboard += clipboard_safe_cell(std::move(value), row < 0 ? std::string{} : narrow(control_text(header_cells[static_cast<std::size_t>(column)])));
            }
            clipboard += L"\r\n";
...
void save_workspace_view() {
    try {
        ...
        point::compliance::write_user_protected_file(temporary_path, output.str());
        if (!MoveFileExW(
...
void copy_all_grid() {
    ...
    const std::size_t maximum_characters =
        50ull * 1024ull * 1024ull / sizeof(wchar_t);
    auto append_row = [&](const std::vector<std::wstring>& values, bool headings = false) {
        for (std::size_t column = 0; column < values.size(); ++column) {
            if (column) clipboard.push_back(L'\t');
            clipboard += clipboard_safe_cell(values[column], headings ? std::string{} : headers[column]);
        }
        clipboard += L"\r\n";
        if (clipboard.size() > maximum_characters)
            throw std::runtime_error(
...
bool copy_active_cell(HWND focus) {
    try {
        ...
        std::string header;
        if (id >= ID_CELL_BASE && id < ID_CELL_BASE + GRID_CONTROL_ROWS * GRID_COLUMNS)
            header = narrow(control_text(header_cells[static_cast<std::size_t>((id-ID_CELL_BASE)%GRID_COLUMNS)]));
        const auto entire = control_text(focus);
        // Inspect the full value even when copying only a selection.
        if (point::looks_like_payment_card_number(point::trim(narrow(entire))))
            throw std::runtime_error("Clipboard blocked: possible payment card number detected.");
        DWORD start = 0, end = 0;
        SendMessageW(focus, EM_GETSEL, reinterpret_cast<WPARAM>(&start), reinterpret_cast<LPARAM>(&end));
        std::wstring chosen = entire;
        if (start < end && end <= entire.size()) chosen = entire.substr(start, end-start);
        if (!write_clipboard_text(clipboard_safe_cell(chosen, header)))
            throw std::runtime_error("Windows clipboard is unavailable.");
        set_status(L"Cell copied with export protections.");
        return true;
    } catch (const std::exception& ex) {
        MessageBoxW(main_window, widen(ex.what()).c_str(), L"Copy blocked", MB_ICONWARNING);
        return false;
    }
}
...
LRESULT CALLBACK window_proc_impl(HWND window, UINT message,
    ...
    case WM_CREATE: {
        ...
        AppendMenuW(workspace_menu, MF_STRING, ID_MENU_FILE_DIFFERENCES,
            L"Reconcile Database vs AD (File Differences)...");
        AppendMenuW(workspace_menu, MF_STRING, ID_MENU_COMPARE_FILES,
            L"Compare Files (Before / After)...");
        AppendMenuW(
    ...
    case WM_COMMAND: {
        ...
        case ID_SCHEDULE_FETCHER:
            ShellExecuteW(main_window, L"open",
                (point::compliance::application_directory() / "PointFetcher.exe").c_str(),
                nullptr, app_root.c_str(), SW_SHOWNORMAL);
            return 0;
        case ID_MENU_COMPARE_FILES:
            point::file_diff_ui::open(main_window, [] { return engine.get(); }, app_root, true);
            return 0;
        case ID_MENU_FILE_DIFFERENCES:
            point::file_diff_ui::open(main_window, [] { return engine.get(); }, app_root);
            return 0;
    ...
    case WM_TIMER:
        if (wparam == INBOX_WATCH_TIMER_ID) {
            if (inbox_change_handle != INVALID_HANDLE_VALUE &&
                WaitForSingleObject(inbox_change_handle, 0) == WAIT_OBJECT_0) {
                if (!FindNextChangeNotification(inbox_change_handle)) {
                    FindCloseChangeNotification(inbox_change_handle);
                    inbox_change_handle = INVALID_HANDLE_VALUE;
                    set_status(L"Inbox watcher stopped; use Refresh to reload reports.");
                }
        ...
        if (wparam == AUTO_SCHEDULE_TIMER_ID) {
            try {
                ...
                if (now - last_retention_run >= 86400) {
                    point::compliance::enforce_retention(app_root, compliance_policy);
        ...
        break;
    case WM_DESTROY:
        KillTimer(window, INBOX_WATCH_TIMER_ID);
        if (inbox_change_handle != INVALID_HANDLE_VALUE) {
            FindCloseChangeNotification(inbox_change_handle);
            inbox_change_handle = INVALID_HANDLE_VALUE;
        }
...
LRESULT CALLBACK window_proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam) {
    try { return window_proc_impl(window, message, wparam, lparam); }
    catch (const std::exception& ex) {
        MessageBoxW(window, widen(ex.what()).c_str(), L"Point operation failed", MB_ICONERROR);
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    try {
        app_root = point::compliance::data_directory();
        const auto installation = point::compliance::application_directory();
        try {
            compliance_policy = point::compliance::load_policy(installation);
        } catch (...) {
            point::startup::recover(installation);
    ...
    if (!window) return 1;
    inbox_change_handle = FindFirstChangeNotificationW((app_root / L"Inbox").c_str(), FALSE,
        FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_LAST_WRITE | FILE_NOTIFY_CHANGE_SIZE);
    if (inbox_change_handle != INVALID_HANDLE_VALUE) SetTimer(window, INBOX_WATCH_TIMER_ID, 500, nullptr);
