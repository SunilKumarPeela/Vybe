...
bool allowed_extension(const std::filesystem::path& path) {
    return point::security_rules::safe_report_name(path.filename().wstring());
}
                                    ...
                                    [](ICoreWebView2*, ICoreWebView2DownloadStartingEventArgs* args) -> HRESULT {
                                        ...
                                        operation->add_BytesReceivedChanged(
                                            Callback<ICoreWebView2BytesReceivedChangedEventHandler>(
                                                [](ICoreWebView2DownloadOperation* download, IUnknown*) -> HRESULT {
                                                    INT64 bytes = 0;
                                                    if (SUCCEEDED(download->get_BytesReceived(&bytes)) && bytes > 500LL * 1024LL * 1024LL)
                                                        download->Cancel();
                                                    return S_OK;
                                                }).Get(), &size_token);
                                        ...
                                        operation->add_StateChanged(
                                            Callback<ICoreWebView2StateChangedEventHandler>(
                                                [destination](ICoreWebView2DownloadOperation* completed_download, IUnknown*) -> HRESULT {
                                                    COREWEBVIEW2_DOWNLOAD_STATE state{};
                                                    completed_download->get_State(&state);
                                                    if (state == COREWEBVIEW2_DOWNLOAD_STATE_COMPLETED) {
                                                        ...
                                                        if (validate_download(destination, reason)) {
                                                            ...
                                                            temporary_path += L".browser.tmp";
                                                            std::filesystem::copy_file(destination, temporary_path,
                                                                std::filesystem::copy_options::overwrite_existing,
                                                                copy_error);
                                                            if (!copy_error && !MoveFileExW(temporary_path.c_str(), final_path.c_str(),
                                                                    MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH))
                                                                copy_error = std::error_code(static_cast<int>(GetLastError()), std::system_category());
                                                            if (copy_error) {
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    try {
        ...
        MessageBoxW(nullptr, L"Browser Fetcher startup denied. Check Windows-group membership, point-security.conf, and data-directory permissions.",
            L"Point Browser Fetcher", MB_ICONERROR);
